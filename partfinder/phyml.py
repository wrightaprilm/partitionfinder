...
def find_program():
    ...
    pathbits = []
    while 1:
        pth, bit = os.path.split(pth)
        if bit == "":
            pathbits.append("/")
            break
        pathbits.append(bit)
    pathbits.reverse()
    log.info(":".join(pathbits))

    # Check if we're frozen inside a package
    if pathbits[-3] == "site-packages.zip":
        pth = os.path.join(*pathbits[:-5])
        pth = os.path.join(pth, _binary_name)
    elif pathbits[-2] == 'partfinder':
        pth = os.path.join(*pathbits[:-2])
        pth = os.path.join(pth, "programs", _binary_name)

    pth = os.path.normpath(pth)
    log.info("Checking for program %s", _binary_name)
